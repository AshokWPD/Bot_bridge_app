...
txtdate=dateformatnow.format(DateTime.now()).toString();
  ...
  var request = http.Request('GET', Uri.parse('https://api.postalpincode.in/pincode/$pinpin'));
                                    ...
                                    ApiClient()
                                      ...
                                      if (response["status"] == 1) {
                                        showDialog(
                                            context: context,
                                            barrierDismissible: false,
                                            builder: (_) => successStatusPOPUP(context,height,width,response["bookingID"],DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now()))
                                        );
                                      }else{
...
void isfill(){
                      ...
                      children: [
                        Padding(
                          ...
                          child: InkWell(
                              onTap: (){
                                print(userNo);
                                NavigateController.pagePush(context, const AddToCartView( bookingType:  "NewBooking", bookID: '', regdate: '',)); //widget.bookingType
                              },
  ...
  Widget successStatusPOPUP(BuildContext context,height,width,bookingID,final regdate){
