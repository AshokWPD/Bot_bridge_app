   ...
   var userno = await LocalDB.getLDB("userID") ?? "";
    ...
    ApiClient().fetchData(url, params).then((response){
            if(response["status"]==1){
              ...
              successStatusPOPUP(context, height, width, "The patient status Updated!", AppointmentView());
 ...
 getData() async {
  ...
  Future<void> scanBarcode(testname, bookingid, sampleno,hight,width) async {
    ...
    if (barcode != '') {
      ...
      conformPOPUP(context, hight, width, "Conform Barcode upload", testname, bookingid, sampleno, barcode);
  ...
  void uploadBarcode(testname, bookingid, sampleno,hight,width,barcodeNum)async{
      Map<String, dynamic> params = {
      "lstBarcode": [
...
//  customAsset: "assets/images/QR-scan.gif",
 ...
 void  conformPOPUP(BuildContext context,double height,double width,content,testname, bookingid, sampleno,barcode){
print("called");
QuickAlert.show(
 context: context,
 type:QuickAlertType.confirm,
